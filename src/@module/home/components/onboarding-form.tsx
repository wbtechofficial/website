  ...
  (
    ...
    const form = useForm({
      defaultValues: {
        ...
        email: "",
        contactNumber: "",
        profession: "Working Professional" as
    ...
    return (
      <div
        className={cn(
          "w-full max-w-md max-h-[85vh] sm:max-h-[500px] overflow-y-auto no-scrollbar rounded-3xl border border-border/80 bg-card shadow-2xl backdrop-blur-lg animate-in fade-in zoom-in-95 duration-300",
          className,
        )}
        ref={ref}
        {...props}
      >
        ...
        <div className="space-y-5 p-5 sm:p-6">
          ...
          <div className="space-y-1.5 text-center">
            <h3 className="font-heading font-bold text-xl sm:text-2xl text-foreground leading-tight tracking-tight">
              {title}
            </h3>
          ...
          >
            ...
            {/* Submit button inside form.Subscribe */}
            <form.Subscribe
              selector={(state) => [state.canSubmit, state.isSubmitting]}
              children={([canSubmit, isSubmittingState]) => (
                <Button
                  type="submit"
                  className="w-full h-10 rounded-xl text-white font-semibold cursor-pointer shadow-sm hover:shadow-md transition-all mt-2"
